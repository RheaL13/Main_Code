...
class MainQuiz:
    ...
    def __init__(self, root, quiz_data, user_info):
        ...
        self.user_info = user_info
        self.q_no = 0  # Current question number
        self.correct = 0  # Correct answers count
        self.display_title()  # Display title of the quiz
        self.display_question()  # Display first question
        self.opt_selected = IntVar()  # Variable to hold selected option
        self.opts = self.radio_buttons()  # Create radio buttons for options
        self.display_options()  # Display options for the first question
        self.buttons()  # Create the next button
...
class QuizSelect:
    ...
    def __init__(self, parent):
        ...
        # Title Label
        title = tk.Label(self.dialog, text="Select the Quiz Area", font=("Helvetica", 16, "bold"), fg="black", bg="oldlace")
        title.pack(pady=30)
...
class DashboardWindow:
    ...
    def show_selected_data(self):
        ...
        if os.path.exists(file_path):
            ...
            # Default content to show if sections are missing
            content_to_display = "No specific data found for this section."

            print(f"User selected: {self.data_selection.get()}")  # Debugging: show dropdown selection
            print("File content:\n", "".join(lines))  # Debugging: show full file content

            # Check if 'Help in Literacy Area(s):' exists in the file
            ...
            # Check if 'Level Info' is selected
            elif "Level Info" in self.data_selection.get():
                print("Level Info selected.")  # Debugging: Confirm this section is selected
                try:
                    start_index = lines.index("Learning Level:\n") + 2
                    end_index = lines.index("Quiz result:", start_index)
                    content_to_display = ''.join(lines[start_index:end_index]).strip()
                except ValueError:
                    content_to_display = "Could not find 'Learning Level:' in file."

            # Check if 'Level Quiz Info' is selected
            elif "Level Quiz Info" in self.data_selection.get():
                print("Level Quiz Info selected.")  # Debugging: Confirm this section is selected
                try:
                    start_index = lines.index("Level Quiz scores:\n") + 2
                    end_index = lines.index("Quiz result:", start_index)
                    content_to_display = ''.join(lines[start_index:end_index]).strip()
                except ValueError:
                    content_to_display = "Could not find 'Level Quiz scores:' in file."

            # Check if 'Quiz Results' is selected
            elif "Quiz Results" in self.data_selection.get():
                print("Quiz Results selected.")  # Debugging: Confirm this section is selected
                try:
                    start_index = lines.index("Quiz result:\n") + 2
                    content_to_display = ''.join(lines[start_index:]).strip()
                except ValueError:
                    content_to_display = "Could not find 'Quiz result:' in file."
